...
Перед сборкой полной страницы открой **`shared/agent-pipeline-pitfalls.md`** (зазор под шапкой, hero, публикация) и **`shared/longread-structure-maslow.md`** (FAQ, кейсы без ссылок).
...
3. **HTML:** `<main id="primary" ...>` → **hero Алины** → контент по уровням **Маслоу** → **блок Бориса** (по якорю) → **FAQ** → CTA.
4. **FAQ-секция:** `id="faq"`, класс **`nero-ai-faq`**, разметка `<details><summary>…</summary><p>…</p></details>` — **все** вопросы из текста Жени (≥12); не сокращать.
5. **Кейсы:** в секции кейсов **не добавляй** `<a href>` и голые URL, даже если они есть в черновике.
6. **`<script>`** — **`longread-page-reveal.js`** (или inline).
7. **JSON-LD:** **FAQPage** — пары вопрос/ответ **1:1** с видимым FAQ на странице; при необходимости Service/WebPage.
...
- **Не вставлять ссылки в блок кейсов** (ни внешние, ни на источники).
- **Не вырезать FAQ** ради краткости — минимум соответствует тексту Жени (12+).
