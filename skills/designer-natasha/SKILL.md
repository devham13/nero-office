...
- **Тёмный hero ${SITE_BRAND}:** по skill Алины hero **по умолчанию тёмный** — класс `nero-ai-hero` **без** `nero-ai-hero--light`. Эталон: `wordpress/page-ai-follow-up-menedzher-povtornye-kasaniya.php`. Подключай `nero_ai_echo_theme_styles()`. В общем `<style>` страницы **не добавляй** белый фон / `--light` на hero. Inline CSS — только page-specific dashboard-виджеты.
