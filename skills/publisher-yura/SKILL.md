...
Если страница выбрана из Google Таблицы (блок Кирилла), **обязательно** обнови **только выбранную строку**:

| Колонка | Значение |
| --- | --- |
| **URL-колонка таблицы** (`GOOGLE_SHEETS_LINK_COLUMN`) | полный публичный URL `https://…/slug/` |
| **Статус** | **`Не использовано`** |

Команда (предпочтительно):

```bash
python3 shared/sheets_mark_published.py --row {номер_строки} --slug {slug}
```

Альтернатива: webhook `GOOGLE_SHEETS_WEBHOOK_URL` или Google Sheets API — но **обе** ячейки (URL + Статус) должны быть заполнены.

**Важно:** колонка URL и колонка **«Статус»** в таблице **не всегда соседние** (между ними может быть пустая колонка). Пиши в **именно** колонку «Статус», не в следующую после URL. Скрипт `sheets_mark_published.py` обновляет две ячейки раздельно.

**Запреты:** не перезаписывать URL в других строках; не менять «Статус» в строках, которые не публиковал в этом запуске.

В блоке `=== ЮРА (ПУБЛИКАЦИЯ) ===` явно укажи: `Google Таблица: строка №{N} — URL ✓, Статус «Не использовано» ✓`.

## После выкладки

1. Запиши в **канонический журнал** `<PROJECT_ROOT>/shared/published-pages.md`: дата, slug, URL, статус.
...
URL: https://...
Статус: опубликовано
Google Таблица: строка №{N} — URL ✓, Статус «Не использовано» ✓
