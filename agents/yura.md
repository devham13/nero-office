...
3. **Собери PHP-шаблон** (`page-{slug}.php`):
   ...
   **OG для соцсетей:** `nero_page_register_social_meta(...)` в шаблоне. После публикации **обязательно**:
   `python3 scripts/capture-page-og-screenshots.py --slug {slug} --apply`
   Скрин первого экрана 1200×630 → `uploads/nero-og-screenshots/{slug}.jpg` + `_nero_og_image`. В HTML `og:image` ≠ фавикон.
