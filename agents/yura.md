...
3. **Собери PHP-шаблон** (`page-{slug}.php`):
   ...
   **OG для соцсетей:** в шаблоне вызывай `nero_page_register_social_meta($page_seo_title, $page_seo_description, $page_og_image)` (см. `shared/templates/current-page-template.php`). Деплой `nero-social-og.php` в mu-plugins + `wp rewrite flush`. В live HTML `og:image` должен быть 1200×630 (`/nero-og/{id}.jpg` или явный URL), не фавикон.
