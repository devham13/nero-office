...
8. **Параллельно в одном сообщении**:
  - **Task**(`alina`) — «Только hero: dashboard справа; класс **`nero-ai-hero` без `nero-ai-hero--light`**; тёмная гамма как у всей страницы ${SITE_BRAND} (эталон `page-ai-follow-up-*.php`). Стили из темы, не белый inline hero. Запиши результат только в `<PROJECT_ROOT>/.cursor/nero-network-fragments/alina.md`; не пиши в handoff.»
