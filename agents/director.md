...
4. **Параллельно в одном сообщении**:
  - **Task**(`seo-kolya`) — «Тема: {тема или победитель Кирилла}. Если есть блок `=== КИРИЛЛ (НОВОСТЬ ДНЯ) ===`, используй его `SEO-вход для Коли`, семена Wordstat, рабочий угол и лидовую гипотезу. Собери семантическое ядро через Wordstat (~10+ вызовов), мета, структуру H2/H3 по **пирамиде Маслоу** (`shared/longread-structure-maslow.md`) + черновик **12–18** FAQ-вопросов. НЕ пиши текст. Запиши результат только в `<PROJECT_ROOT>/.cursor/nero-network-fragments/kolya.md`; не пиши в handoff.»
...
6. **Task**(`zhenya`) — «Прочитай блоки Коли и Артёма, а если есть — блок Кирилла. Лонгрид 8k–20k+ по **Маслоу** (`shared/longread-structure-maslow.md`): кейсы **без ссылок/URL**, FAQ **12–18** развёрнутых Q&A под нейровыдачу. Сохрани мета, `SLUG`, `H1_для_hero`, `ПОДЗАГОЛОВОК_HERO`. Обнови handoff блоком `=== ЖЕНЯ (ЛОНГРИД) ===`, не удаляя служебные данные».
