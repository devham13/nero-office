...
3. Создай hero-блок под тему. **Каждый раз проектируй новый мир / пространство / центральную метафору**, а не вариацию старой сцены.
4. **Разметка hero** — `<section class="nero-ai-hero">` **без** `nero-ai-hero--light`. Стили базовые — из `nero-ai-*` (эталон: `wordpress/page-ai-follow-up-menedzher-povtornye-kasaniya.php`). Inline CSS — только для уникальных виджетов dashboard, **не** для белого фона hero.
...
- **Не полагаться на CSS темы** — базовые стили hero через `nero-ai-*`; inline только для page-specific dashboard.
- **Не использовать `nero-ai-hero--light`** и белый inline-фон на hero.
