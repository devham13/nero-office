<?php
/**
 * Юридические константы и хелперы сайта (оператор, политика, футер).
 * Подключать из MU-plugins, bootstrap лонгридов и page-{slug}.php.
 */
...
if (!function_exists('nero_site_operator_line')) {
    function nero_site_operator_line(): string
    {
        return sprintf(
            'Оператор: %s, ИНН %s',
            NERO_SITE_OPERATOR_NAME,
            NERO_SITE_OPERATOR_INN
        );
    }
}

if (!function_exists('nero_site_privacy_consent_line')) {
    /** Текст рядом с CTA / формами обратной связи. */
    function nero_site_privacy_consent_line(): string
    {
        $privacy = nero_site_privacy_url();
        return 'Отправляя обращение, вы соглашаетесь с '
            . '<a href="' . esc_url($privacy) . '">политикой конфиденциальности</a>. '
...
if (!function_exists('nero_site_render_legal_footer_block')) {
    /**
     * Юридический блок перед get_footer() на лонгридах (дублирует MU-plugin для визуальной связки с контентом).
     */
