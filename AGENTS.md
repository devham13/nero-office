...
- **«Запусти проект»** = полный пайплайн до **URL**: Кирилл → … → Наташа → **Юра** → Макс||Лёня. **Не останавливаться** после Наташи и **не ждать** отдельной команды «опубликуй».
- **Не спрашивай** подтверждение на deploy, SSH/FTP, QA, SEO-аудит, повторную публикацию.
- **Не останавливайся** на «могу продолжить?» — доводи до **URL** или **блокера с причиной**.
- Если Task прерван: директор дожимает `deploy.py`, curl, QA/SEO фрагменты.
- После **Юры** всегда **Макс||Лёня**.
