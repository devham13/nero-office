...
- **Не спрашивай** подтверждение на шаги **создания** (Кирилл…Наташа).
- **Не останавливайся** на «могу продолжить?» в фазе создания — доводи до `wordpress/page-{slug}.php`, блока `=== НАТАША (HTML СТРАНИЦЫ) ===` ✅, **коммита** в Git, затем **остановись** (slug + путь к шаблону).
- **Не запускай Юру / QA / SEO** после Наташи без явной команды на публикацию.
- В **фазе публикации** не спрашивай подтверждение на deploy, SSH, QA, IndexLift — доводи до **URL** или блокера.
- Если Task прерван в фазе публикации: директор может дожать `deploy.py`, curl, IndexLift; после Юры — **Макс||Лёня**.
