...
.ncai-related{
  margin-top:1.25em;padding:14px 16px;border-radius:12px;
  background:rgba(99,102,241,.08);border:1px solid rgba(99,102,241,.18);
}
.ncai-related-title{
  font-size:13.5px;line-height:1.55;color:var(--ncai-muted);margin:0 0 10px;
}
.ncai-related-list{
  list-style:none;margin:0;padding:0;display:flex;flex-direction:column;gap:8px;
}
.ncai-related-list a{
  font-size:14px;font-weight:600;color:var(--ncai-accent);text-decoration:none;
}
.ncai-related-list a:hover{text-decoration:underline;}
.ncai-intro-kpi{
    ...
    <div class="ncai-cnt nero-ai-container">
      <div class="ncai-intro-grid nero-ai-intro-grid nero-ai-reveal">
        <div class="ncai-intro-text nero-ai-intro-text">
          ...
          <nav class="ncai-related nero-ai-reveal" aria-label="Смежные материалы по внедрению AI">
            <p class="ncai-related-title"><strong>Смежные сценарии:</strong> узкие посадочные дополняют общий no-code подход — CRM, почта, ERP и корпоративные кейсы.</p>
            <ul class="ncai-related-list">
              <li><a href="/vnedrenie-ai-amocrm/">внедрение AI-агента для amoCRM под ключ</a></li>
              <li><a href="/vnedrenie-ai-obrabotka-email-crm/">автоматизация обработки входящей почты в CRM</a></li>
              <li><a href="/ai-1c-erp/">AI-агент для 1С и ERP: заявки, счета, документы</a></li>
              <li><a href="/kpmg-claude-vnedrenie-ai-276-tysyach/">масштабное внедрение AI: уроки KPMG и Claude</a></li>
            </ul>
          </nav>
        </div>
...
<?php
$ncai_page_url  = trailingslashit(get_permalink());
$ncai_site_url  = trailingslashit(home_url('/'));
$ncai_page_h1   = 'No-code внедрение AI для бизнеса: автоматизация под ключ без программистов';
$ncai_schema    = [
    '@context' => 'https://schema.org',
    '@graph'   => [
        [
            '@type' => 'Organization',
            '@id'   => $ncai_site_url . '#organization',
            'name'  => $brand,
            'url'   => $ncai_site_url,
        ],
        [
            '@type'     => 'WebSite',
            '@id'       => $ncai_site_url . '#website',
            'url'       => $ncai_site_url,
            'name'      => $brand,
            'publisher' => ['@id' => $ncai_site_url . '#organization'],
        ],
        [
            '@type'       => 'WebPage',
            '@id'         => $ncai_page_url . '#webpage',
            'url'         => $ncai_page_url,
            'name'        => $ncai_page_h1,
            'description' => $page_seo_description,
            'isPartOf'    => ['@id' => $ncai_site_url . '#website'],
            'about'       => ['@id' => $ncai_site_url . '#organization'],
        ],
        [
            '@type' => 'BreadcrumbList',
            '@id'   => $ncai_page_url . '#breadcrumb',
            'itemListElement' => [
                ['@type' => 'ListItem', 'position' => 1, 'name' => 'Главная', 'item' => $ncai_site_url],
                ['@type' => 'ListItem', 'position' => 2, 'name' => $ncai_page_h1, 'item' => $ncai_page_url],
            ],
        ],
        [
            '@type'       => 'Service',
            '@id'         => $ncai_page_url . '#service',
            'name'        => $ncai_page_h1,
            'description' => $page_seo_description,
            'url'         => $ncai_page_url,
            'provider'    => ['@id' => $ncai_site_url . '#organization'],
        ],
        [
            '@type' => 'FAQPage',
            '@id'   => $ncai_page_url . '#faq',
            'mainEntity' => [
                ['@type' => 'Question', 'name' => 'Как внедрить no code ai для бизнеса?', 'acceptedAnswer' => ['@type' => 'Answer', 'text' => 'Аудит 1–2 процессов → FAQ и 20–50 диалогов → пилот на одном канале + CRM → метрики 2–4 недели → масштабирование. Под ключ: 2–6 недель.']],
                ['@type' => 'Question', 'name' => 'Сколько стоит no code ai для бизнеса?', 'acceptedAnswer' => ['@type' => 'Answer', 'text' => 'DIY от 3–10 тыс. ₽/мес; пилот 80–150 тыс. ₽; рабочий агент 150–350 тыс. ₽; мультиагент 300–500 тыс. ₽.']],
                ['@type' => 'Question', 'name' => 'Под ключ или самостоятельно?', 'acceptedAnswer' => ['@type' => 'Answer', 'text' => 'DIY — если есть время на эксперименты. Под ключ — если нужен результат, интеграции и метрики с первого дня.']],
                ['@type' => 'Question', 'name' => 'No-code AI с CRM — как связать?', 'acceptedAnswer' => ['@type' => 'Answer', 'text' => 'Канал → AI-агент → webhook Make/n8n → amoCRM/Bitrix24 (лид, задача, поля). Для e-commerce: + RetailCRM, «МойСклад».']],
                ['@type' => 'Question', 'name' => 'Нужен ли программист?', 'acceptedAnswer' => ['@type' => 'Answer', 'text' => 'Нет для типовых сценариев. Low-code — точечный код для сложных API; основная сборка визуальная.']],
                ['@type' => 'Question', 'name' => 'Можно ли начать с одного канала?', 'acceptedAnswer' => ['@type' => 'Answer', 'text' => 'Да — рекомендуемая стратегия: Telegram + CRM + база знаний за 2–3 недели, затем масштаб по метрикам.']],
            ],
        ],
    ],
];
echo '<script type="application/ld+json">' . wp_json_encode($ncai_schema, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES) . '</script>' . "\n";
?>
