...
| Можно ли подавать на перепроверку? | **Да** — ИНН и оператор заполнены; осталось GSC/Яндекс перепроверка |
...
| Оператор и ИНН | `nero-site-legal.php` + страницы 122–125 | Горбачев Ю.А., ИНН 164807872488, политика 152-ФЗ (ред. 08.06.2026) | **Задеплоено 18:31 MSK** |
| Юридический футер на всех страницах | `wp_footer` в MU-plugin | Ссылка на политику, контакты, условия, строка оператора | **Задеплоено** |
| Правило для новых страниц | `rules/site-legal-compliance.mdc` + `AGENTS.md` | Обязательный `get_footer()` и ссылка на политику | **В репозитории** |
...
| Юридические реквизиты | Заполнено | Горбачев Ю.А., ИНН 164807872488, devham@mail.ru |
...
1. ~~**Заполнить реквизиты**~~ — **выполнено** (Горбачев Ю.А., ИНН 164807872488).
...
- [x] Email/ИНН на страницах доверия (Горбачев Ю.А., 164807872488)
